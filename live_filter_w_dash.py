...
f.write(str(fill) + "; " + str(time.time()) + "; " + str(temp_read0) + "; " + str(temp_read1) + "; " + str(
...
def calc():
    ...
    # FFT
    y0f = scipy.fft.fft(y0_offsetfree)
    y1f = scipy.fft.fft(y1_offsetfree)
    y2f = scipy.fft.fft(y2_offsetfree)
    y3f = scipy.fft.fft(y3_offsetfree)

    # Sum
    yf = y0f + y1f + y2f + y3f

    # IFFT
    y_total = scipy.fft.ifft(yf)
